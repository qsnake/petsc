...
extern "C" {
#endif
void PETSC_STDCALL  pcgamgsetnsmooths_(PC pc,PetscInt *n, int *__ierr ){
*__ierr = PCGAMGSetNSmooths(
	(PC)PetscToPointer((pc) ),*n);
}
void PETSC_STDCALL  pcgamgsetsymgraph_(PC pc,PetscBool *n, int *__ierr ){
*__ierr = PCGAMGSetSymGraph(
	(PC)PetscToPointer((pc) ),*n);
}
void PETSC_STDCALL  pcgamgsetsquaregraph_(PC pc,PetscBool *n, int *__ierr ){
